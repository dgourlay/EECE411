...
def doUpdate(mine, other) :
	...
	for n in nodes :
		if not mine.has_key(n) :
			...
			# compare their rev # against your own
			...
			m_rev = m_nodeStatus[0]
			o_rev = o_nodeStatus[0]
			
			if m_rev < o_rev : # their revision number is higher
				# update the local map entry
				mine.update({n:o_nodeStatus})
			elif m_rev == o_rev : # revision numbers are the same
				...
				if m_dead != o_dead :
					...
					if o_dead == 1 :
						# update the local map entry
						mine.update({n:o_nodeStatus})
			else : # local revision number is higher
				# update their map entry
				other.update({n:m_nodeStatus})
	# end for

# increments rev by 1 and update map with system status
# note this function operates directly on 'mine'
def updateOwnStatus(mine, rev):
	ownStatus = [rev, 0,
		stats.getCapacity(),
